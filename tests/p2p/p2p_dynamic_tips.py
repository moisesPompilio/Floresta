...
# Number of alternative tips to create, one fork per height starting at 1
NUM_FORKS = 75
...
def test_dynamic_chain_tips_derivation_restart(
    ...
    # 2. Collect parent headers from bitcoind at heights 1..NUM_FORKS
    ...
    parent_headers = [
        ...
        for h in range(1, NUM_FORKS + 1)
    ]

    # 3. Create NUM_FORKS independent fork headers via CBlock.solve()
    log.info("Creating %d fork headers", NUM_FORKS)
    fork_headers = [create_fork_header(p) for p in parent_headers]
    fork_hashes = [fh.hash_hex for fh in fork_headers]
    assert (
        len(set(fork_hashes)) == NUM_FORKS
    ), f"Expected {NUM_FORKS} distinct fork hashes"
