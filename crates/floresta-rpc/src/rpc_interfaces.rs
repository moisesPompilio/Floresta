...
pub trait BlockchainRpc {
    ...
    /// scriptPubKey of this utxo. It returns `None` if the  utxo doesn't exist. You must have
    /// enabled block filters by setting the `blockfilters=1` option.
    fn find_tx_out(
        &self,
        txid: Txid,
        vout: u32,
...
pub trait ControlRpc {
    ...
    fn get_memory_info(&self, mode: Option<&str>) -> Result<GetMemInfoRes, Self::Error>;
