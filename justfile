...
# List of crates of floresta. Requires `jq` to be installed (checked by `run-each`).
...
run-each before_p after_p: (check-command "jq" "run-each" "https://jqlang.github.io/jq/download/")
    #!/usr/bin/env bash
    set -e
    echo "Crates {{ _crates }} "
